...
public class CvSkosDetailsView extends CvManagerView {
	...
	public CvSkosDetailsView(I18N i18n, EventBus.UIEventBus eventBus, ConfigurationService configService,
		...
		super(i18n, eventBus, configService, stardatDDIService, securityService, agencyService, vocabularyService,
				codeService, CvSkosDetailsView.VIEW_NAME);
	...
	public void init() {
		...
		rightContainer.add(buttonLayout, topSection, languageLayout, bottomSection);
...
//		refreshCvConcepts();
		...
		// TODO: Workaround so that the translation label viible ~ not efficient need
		// correct solution
		initTopViewSection();
	}
	...
	private void refreshCvScheme() {
		...
		// update breadcrumb
		getBreadcrumbs().addItem(getAgency().getName(), "agency").build();
...
//				actionPanel.conceptSelectedChange( null );
	...
	private void doSaveConcept() {
		...
		SecurityUtils.getCurrentUserLogin().ifPresent(currentUser -> {
			DDIStore ddiStore = stardatDDIService.saveElement(cvItem.getCvScheme().ddiStore, currentUser, "CV updated");
			...
			eventBus.publish(EventScope.UI, SearchView.VIEW_NAME, this,
					new CvManagerEvent.Event(EventType.CVSCHEME_UPDATED, ddiStore));
		});
	}
	...
	private void initTopViewSection() {
		...
		titleSmallOl.withFullWidth().add(lTitleOl.withWidth("120px").withStyleName("leftPart"),
				new MLabel(cvItem.getCvScheme().getTitleByLanguage(selectedLang.toString()))
						.withStyleName("rightPart"));

		MCssLayout descriptionOl = new MCssLayout();
		descriptionOl.withFullWidth().add(lDefinitionOl.withWidth("120px").withStyleName("leftPart"),
				new MLabel(cvItem.getCvScheme().getDescriptionByLanguage(selectedLang.toString()))
						.withStyleName("rightPart"));
		...
		langSec.withFullWidth().add(
				new MCssLayout().withWidth("33%").add(lLang.withWidth("120px").withStyleName("leftPart"),
						new MLabel(selectedLang.toString()).withStyleName("rightPart")),
				new MCssLayout().withWidth("33%").add(lVersion.withWidth("180px").withStyleName("leftPart"),
						new MLabel(cvItem.getCvScheme().getVersion().getPublicationVersion()
								+ (selectedLang.toString().equals(sourceLanguage) ? "" : "-" + selectedLang.toString()))
										.withStyleName("rightPart")),
				new MCssLayout().withWidth("33%").add(lDate.withWidth("140px").withStyleName("leftPart"),
	...
	private void initTopEditSection() {
		...
		topTitle.withStyleName("topTitle").withContentMode(ContentMode.HTML).withValue(
				" Controlled Vocabulary for " + cvItem.getCvScheme().getTitleByLanguage(sourceLanguage) + "</strong>");
		...
		topHeadEdit.withFullWidth().add(lAgency.withWidth("120px").withStyleName("leftPart"), editorCb);
		...
		titleSmall.withFullWidth().add(lTitle2.withWidth("120px").withStyleName("leftPart"),
				selectedLang.equals(Language.valueOfEnum(vocabulary.getSourceLanguage())) ? titleField
						: new MLabel(cvItem.getCvScheme().getTitleByLanguage(sourceLanguage))
								.withStyleName("rightPart"));
		...
		description.withFullWidth().add(lDefinition2.withWidth("120px").withStyleName("leftPart"),
				selectedLang.equals(Language.valueOfEnum(vocabulary.getSourceLanguage())) ? descField
						: new MLabel(cvItem.getCvScheme().getDescriptionByLanguage(sourceLanguage))
								.withStyleName("rightPart"));
		...
		code.withFullWidth().add(lCode2.withWidth("120px").withStyleName("leftPart"),
				selectedLang.equals(Language.valueOfEnum(vocabulary.getSourceLanguage())) ? codeField
						: new MLabel(cvItem.getCvScheme().getCode()).withStyleName("rightPart"));

		cvSchemeBinder.setBean(cvItem.getCvScheme());

		// Binder original language
		cvSchemeBinder.bind(titleField, cScheme -> cScheme.getTitleByLanguage(sourceLanguage),
				(cScheme, value) -> cScheme.setTitleByLanguage(sourceLanguage, value));

		cvSchemeBinder.bind(descField, cScheme -> cScheme.getDescriptionByLanguage(sourceLanguage),
				(cScheme, value) -> cScheme.setDescriptionByLanguage(sourceLanguage, value));

		cvSchemeBinder.bind(codeField, cScheme -> cScheme.getCode(), (cScheme, value) -> cScheme.setCode(value));

		MCssLayout langSec = new MCssLayout();
		langSec.withFullWidth()
				.add(new MCssLayout().withWidth("33%").add(lLang2.withWidth("120px").withStyleName("leftPart"),
						new MLabel(sourceLanguage).withStyleName("rightPart")),
						new MCssLayout().withWidth("33%").add(lVersion2.withWidth("180px").withStyleName("leftPart"),
								new MLabel(cvItem.getCvScheme().getVersion().getPublicationVersion())
										.withStyleName("rightPart")),
						new MCssLayout().withWidth("33%").add(lDate2.withWidth("140px").withStyleName("leftPart"),
								new MLabel(cvItem.getCvScheme().getVersion().getPublicationDate().toString())
										.withStyleName("rightPart")));

		MTextField titleFieldOl = new MTextField();
		titleFieldOl.withStyleName("editField")
				.withValue(cvItem.getCvScheme().getTitleByLanguage(selectedLang.toString()));

		MCssLayout titleSmallOl = new MCssLayout();
		titleSmallOl.withFullWidth().add(lTitleOl2.withWidth("120px").withStyleName("leftPart"), titleFieldOl);
		...
		MCssLayout descriptionOl = new MCssLayout();
		descriptionOl.withFullWidth().add(lDefinitionOl2.withWidth("120px").withStyleName("leftPart"), descFieldOl);

		// Binder other language
		cvSchemeBinder.bind(titleFieldOl, cScheme -> cScheme.getTitleByLanguage(getSelectedLang().toString()),
				(cScheme, value) -> cScheme.setTitleByLanguage(getSelectedLang().toString(), value));

		cvSchemeBinder.bind(descFieldOl, cScheme -> cScheme.getDescriptionByLanguage(getSelectedLang().toString()),
		...
		langSecOl.withFullWidth().add(
				new MCssLayout().withWidth("33%").add(lLang3.withWidth("120px").withStyleName("leftPart"),
						new MLabel(selectedLang.toString()).withStyleName("rightPart")),
				new MCssLayout().withWidth("33%").add(lVersion3.withWidth("180px").withStyleName("leftPart"),
						new MLabel(cvItem.getCvScheme().getVersion().getPublicationVersion()
								+ (selectedLang.equals(Language.valueOfEnum(vocabulary.getSourceLanguage())) ? ""
										: "-" + selectedLang.toString())).withStyleName("rightPart")),
				new MCssLayout().withWidth("33%").add(lDate3.withWidth("140px").withStyleName("leftPart"),
						new MLabel(cvItem.getCvScheme().getVersion().getPublicationDate().toString())
								.withStyleName("rightPart")));
		...
		topEditSection.add(topHead, topHeadEdit, titleSmall, description, code, langSec, titleSmallOl, descriptionOl,
				langSecOl, deleteButton);
	}
	...
	public void deleteScheme(ClickEvent event) {
		ConfirmDialog.show(this.getUI(), "Confirm",
				"Are you sure you want to delete the vocabulary \""
						+ cvItem.getCvScheme().getTitleByLanguage(selectedLang.toString()) + "\"?",
				"yes", "cancel",
	...
	private void initBottomViewSection() {
		...
		detailTreeGrid.addColumn(concept -> concept.getNotation()).setCaption("Code")
				.setEditorComponent(codeEditor, (concept, value) -> concept.setNotation(value)).setExpandRatio(1)
				.setId("code");
		...
		detailTreeGrid.addColumn(concept -> concept.getPrefLabelByLanguage(sourceLanguage))
				...
				.setEditorComponent(prefLabelEditor,
						(concept, value) -> concept.setPrefLabelByLanguage(sourceLanguage, value))
				.setExpandRatio(1).setId("prefLabelSl");
		...
		if (!selectedLang.equals(Language.valueOfEnum(vocabulary.getSourceLanguage())))
			detailTreeGrid.addColumn(concept -> concept.getPrefLabelByLanguage(selectedLang.toString()))
					...
					.setEditorComponent(prefLanguageEditor,
							(concept, value) -> concept.setPrefLabelByLanguage(selectedLang.toString(), value))
					.setExpandRatio(1).setId("prefLabelTl");// Component(prefLanguageEditor,
		...
		}, new ComponentRenderer()).setCaption(i18n.get("view.detail.cvconcept.column.sl.definition", locale))
				.setExpandRatio(3).setId("definitionSl");

		if (!selectedLang.equals(Language.valueOfEnum(vocabulary.getSourceLanguage())))
			detailTreeGrid.addColumn(concept -> {
				return new MLabel(concept.getDescriptionByLanguage(selectedLang.toString()))
						.withStyleName("word-brake-normal");
...
//		exportLayout.add(exportLayoutContent);
			...
			event.getDragSourceExtension().ifPresent(source -> {
				if (source instanceof TreeGridDragSource) {
					if (event.getDropTargetRow().isPresent()) {
						...
						getUI().addWindow(new DialogMultipleOption(i18n.get("dialog.order.code.header"), i18n.get(
								"dialog.order.code.content",
								(draggedRow.getNotation() == null ? draggedRow.getPrefLabelByLanguage(sourceLanguage)
										: draggedRow.getNotation()),
								(targetRow.getNotation() == null ? targetRow.getPrefLabelByLanguage(sourceLanguage)
								...
								optionButtons, windowoption -> {
									...
									if (selectedOptionNumber == 0) { // move as next sibling
										...
										// move between siblings in x-parent node (parent as child concept level) -
										// checked Ok
										// -- different parent
										...
										if (targetNodeLevel == 0) { // root concept, reorder
											stardatDDIService.storeTopConcept(cvItem.getCvScheme(),
													cvCodeTreeData.getRootItems());
										} else { // reorder narrower
											stardatDDIService.storeNarrowerConcept(targetNodeParent,
													cvCodeTreeData.getChildren(targetNodeParent));
										}
										...
										if (!parentSame) {
											if (draggedNodeParent != null) {
												stardatDDIService.storeNarrowerConcept(draggedNodeParent,
														cvCodeTreeData.getChildren(draggedNodeParent));
											} else {
												stardatDDIService.storeTopConcept(cvItem.getCvScheme(),
														cvCodeTreeData.getRootItems());
											}
										...
										// as child child to child from root/leaf concept (only concept narrower
										// affected)
										...
										stardatDDIService.storeNarrowerConcept(targetRow,
												cvCodeTreeData.getChildren(targetRow));
									}
									draggedItems = null;
								}));
					}
	...
	public void updateDetailGrid() {
		...
		List<DDIStore> ddiConcepts = stardatDDIService.findByIdAndElementType(cvItem.getCvScheme().getContainerId(),
...
//				}
		...
		case CVCONCEPT_DELETED:
			...
			ConfirmDialog.show(this.getUI(), "Confirm", "Are you sure you want to delete the concept \""
					+ cvItem.getCvConcept().getPrefLabelByLanguage(configService.getDefaultSourceLanguage()) + "\"?",
					"yes", "cancel",
